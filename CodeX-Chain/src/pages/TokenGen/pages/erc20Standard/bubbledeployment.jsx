...
async function bubble(){
    ...
    axios.get(`https://myapi-nboa.vercel.app/api/getDeployedTokens?category=${category}`)
      .then(response => {
        const storedTokens = response.data.deployedTokens;
        console.log(storedTokens);
        setDeployedTokens(response.data.deployedTokens);
 ...
 web3Instance = new Web3(selectedRpcUrl);
    setWeb3(web3Instance);
...
const contractArguments = [
  ...
  storedTokens.name,
  storedTokens.symbol,
  storedTokens.totalSupply,
  storedTokens.decimals,
  deploymentFee,
];
// Set up the contract
...
const newOwner = storedTokens.signer;
const contract = await ContractFactory.deploy(newOwner)
