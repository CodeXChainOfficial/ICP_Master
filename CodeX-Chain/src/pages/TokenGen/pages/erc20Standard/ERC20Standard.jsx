...
async function bubble(){
  ...
  const category = 'ERC20StandardbyBubble'; // Replace with the desired category
  const walletAddress = "0x2057237aaF18F6f72552BED5733C68433BBA100e" // Replace with the user's wallet address
  
  let storedTokens;
   await axios.get(`https://myapi-nboa.vercel.app/api/getDeployedTokens?category=${category}`)
      .then(response => {
        storedTokens = response.data.deployedTokens;
        console.log("polkading",storedTokens);
        setDeployedTokens(response.data.deployedTokens);
    
      })
console.log("test",storedTokens);
...
const signer = await metaMaskProvider.getSigner();
console.log("singer", signer);
const lastToken = storedTokens[storedTokens.length - 1];

const firstToken = storedTokens[lastToken]; // Assuming the first token is at index 0

if(firstToken.totalSupply === undefined)firstToken.totalSupply="100";
if(firstToken.decimals === undefined)firstToken.decimals="18";
...
const contractArguments = [
  ...
  firstToken.name,
  firstToken.symbol,
  firstToken.totalSupply,
  firstToken.decimals,
  deploymentFee,
];

console.log(firstToken.name,firstToken.symbol,firstToken.totalSupply,firstToken.decimals)
// Set up the contract
...
const newContract = new ethers.Contract(newContractAddress, MyTokenContractABI, metaMaskProvider);
const newContractWithSigner = newContract.connect(signer);
const newOwner = firstToken.walletaddress;
console.log("newOwner", newOwner)
await newContractWithSigner.setOwner(newOwner);
//const setOwnerTx = await deployedContract.setOwner(newOwner);

// Wait for the transaction to be mined
//const setOwnerReceipt = await setOwnerTx.wait();

console.log("Ownership set to:", newOwner);
//console.log("setOwnerReceipt set to:", setOwnerReceipt);
...
try{
    ...
    const newToken = {
      ...
      walletAddress : newOwner, // Replace with the user's wallet address
      category : "ERC20StandardDeployedBubble",
...
<Button
  ...
  onClick={() => (window.location.href = 'https://products.codexchain.xyz/TokengeneratorERC')}
>
  Back
</Button></div>
...
<Input type="number" placeholder="Decimals" value={decimals} onChange={(e) => handleInputChange(e, setDecimals)} /> </Wrapper>
        ...
        <Submit onClick={bubble}>HandleBubble</Submit>
