...
export default function DIP20Standard() {
  ...
  const handleCreateID = async () => {
    try {
      const response = await fetch('https://codexdev.tech/api/identity', {
        method: 'POST',
        headers: {
...
const deploycansiter = async () => {
  ...
  axios.post('https://codexdev.tech/api/saveDeployedTokens', { deployedTokens: [newToken] })
  .then(response => {
  ...
  axios.get(`https://codexdev.tech/api/getDeployedTokens?category=${category}&walletAddress=${walletAddress}`)
    .then(response => {
  ...
  useEffect(() => {
    const fetchTotalCounts = async () => {
      try {
        ...
        const apiUrl = 'https://codexdev.tech/api/getDeployedTokensCount';
